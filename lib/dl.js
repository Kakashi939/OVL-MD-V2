...
async function fbdl(url, maxRetries = 5) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      const { data } = await axios.post('https://getmyfb.com/process', `id=${encodeURIComponent(url)}&locale=id`, {
        headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': 'Mozilla/5.0', 'Referer': 'https://getmyfb.com/id' },
    });
    const $ = cheerio.load(data);
    const link = $(".results-list li a").map((_, el) => $(el).attr("href")).get();
    return { link
    };
    } catch (error) {
